...
function updateDefaultLogData() {
  ...
  try {
    var _pkg$repository;
    const pkg = JSON.parse(readFileSync(nodePath.join(process.cwd(), 'package.json')).toString());
      ...
      compiler.hooks.done.tapAsync(pluginName, (compilation, callback) => {
        this.arr.forEach(path => {
          if (this.clientReg.exec(path)) {
            ...
            const htmlpath = nodePath.join(path1, 'dist', 'server', 'index.spa.html');
          ...
          if (this.serverReg.exec(path)) {
            ...
            const htmlpath = nodePath.join(path1, 'dist', 'server', 'index.ssr.html');
            const str = readFileSync(htmlpath).toString();
            const str1 = str.replace('</head>', `<script>${info}</script></head>`);
            writeFileSync(htmlpath, str1);
          }
...
export { BilldHtmlWebpackPlugin as default, logData, version };
